...
fol='C:\Users\HUAWEI\Desktop\QZ_L2&solution\QZ_L2&solution\20240401-0420';
f=fullfile(fol,'*.txt');
...
%for i = 1:length(fD)
i=6;
    st = cell2mat(fD{i}(4,:));   %%% fD{2}(4,:)  fD{6}(4,:)
    ...
    % end
     ...
     % % 筛选0km到40km的数据
    height_limit = 40; % 设置高度限制
    valid_indices = sh <= height_limit; % 找到满足条件的索引

    % 筛选数据
    C_filtered = C(valid_indices, :); % 只保留0km到40km的数据
    sh_filtered = sh(valid_indices); % 只保留对应的高度
    sx_filtered = sx(valid_indices, :); % 只保留对应的纬向风
    sy_filtered = sy(valid_indices, :); % 只保留对应的经向风
    sv_filtered = sv(valid_indices, :); % 只保留对应的垂直风

    % %
    if length(st)>1 && ~isempty(sh_filtered)
         
        figure;
        til = tiledlayout(3,1);
        
        ss_filtered = {sx_filtered, sy_filtered, sv_filtered};
        tex = ["纬向风","经向风","垂直风"];
        for k = 1:3
            nexttile;
            contourf(st, sh_filtered, ss_filtered{k}, 20);
            colormap default;
            colorbar;
            clim([-50,50]);
            title(tex(k));
        ...
        r3='MHz; ';
        r4='HR=';
        r5=num2str((sh_filtered(2)-sh_filtered(1))*1000); % 计算新的高度间隔
        r6='m';
